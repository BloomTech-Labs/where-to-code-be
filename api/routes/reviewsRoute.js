...
router.get("/:id/user", authenticate, async (req, res) => {
  try {
    ...
    if (reviewUser) {
      ...
      res
        .status(400)
        .send({ message: "User for this review is not found", error });
    }
  } catch (err) {
    res.status(500).json({ message: "Error fetching review user", err });
  }
});
...
router.get("/:id/location/:userid", async (req, res) => {
  try {
    ...
    const reviewLocation = await REVIEW_MODEL.getReviewsByLocationUser(
      id,
      userid
    );
    console.log("rl", reviewLocation);
